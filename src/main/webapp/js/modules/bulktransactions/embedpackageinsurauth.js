...
function getBulkPolicy() {
    var url = 'viewunprocessembedpackage';
    var currTable = $('#loadedPolicyTable').DataTable({
        ...
        "columns": [
            ...
            },
            {
                "data": null,
                "render": function (data, type, full, meta) {
                    return (full.insuredFName || '') + ' ' + (full.insuredOtherNames || '');
                }
            },
            {
                "data": "polWef",
            ...
            // },
            {
                "data": "polId",
                "render": function (data, type, full, meta) {
                    return `<button type="button" class="btn btn-primary btn-sm" data-trans=${encodeURI(JSON.stringify(full))} value="Process" onclick="authSingleEmbedPackage(this);">Authorize</button>`;
                }
            }
        ],
        ...
        "initComplete": function () {
            ...
            $('#bulk-authorize-policy').on('click', function () {
                ...
                bulkEmbedPackageAuth(checkedPolicy, currTable);
            });
...
function authSingleEmbedPackage(button) {
    ...
    bootbox.confirm("Are you sure want to authorize this policy?", function (result) {
        ...
        if (result) {
            Swal.fire({
                title: 'Processing...',
                text: 'Please wait while we authorize the policies.',
            ...
            $.ajax({
                type: 'POST',
                url: 'approvesinglembedinsur/' + trans['polId'],
                dataType: 'json',
                async: true,
...
function bulkEmbedPackageAuth(policyId, tableInstance) {
...
function bulkProcessDelTransactions(policyId, tableInstance) {
    ...
    bootbox.confirm("Are you sure you want to bulk Delete selected transactions?", function (result) {
        if(result){
            ...
            $.ajax({
                url: 'processbulkdelprocessedpolicies',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(policyId.map(Number)),
                success: function (response) {
