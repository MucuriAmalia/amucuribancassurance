$(function () {
    $(document).ready(function () {
        getBulkGroupLifePolicy();
    });
});

function getBulkGroupLifePolicy() {
    var url = 'viewunprocessgrouplife';
    var currTable = $('#loadedPolicyTable').DataTable({
        ...
        "columns": [
            ...
            {"data": "polNo"},
            {
                "data": "totPremium",
                "render": function (data, type, full, meta) {
                    return UTILITIES.currencyFormat(full.totPremium);
                }
            },
            {
                "data": null,
            ...
            // },
            {
                "data": "polId",
                "render": function (data, type, full, meta) {
                    return `<button type="button" class="btn btn-primary btn-sm" data-trans=${encodeURI(JSON.stringify(full))} value="Process" onclick="processGroupLife(this);">Authorize</button>`;
                }
            }
        ],
        ...
        "initComplete": function () {
            ...
            $('#bulk-authorize-policy').on('click', function () {
                ...
                bulkGroupLifeAuth(checkedPolicy, currTable);
            });
...
function processGroupLife(button) {
    ...
    bootbox.confirm("Are you sure want to authorize this policy?", function (result) {
        ...
        if (result) {
            ...
            $.ajax({
                type: 'POST',
                url: 'approvesinglegrouplife/' + trans['polId'],
                dataType: 'json',
                async: true,
...
function bulkGroupLifeAuth(policyId, tableInstance) {
