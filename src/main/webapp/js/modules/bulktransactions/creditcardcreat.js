$(function () {
    $(document).ready(function () {
        ...
        getUnprocessedCreditCard();
    });
});

// function importCreditCardData() {
...
function getUnprocessedCreditCard() {
    var url = "unprocesscreditcard";
    var currTable = $('#uploadedTransTable').DataTable({
        ...
        "rowCallback": function (row, data) {
            ...
            checkbox.prop('checked', checkedRows[data.cardId] || false);
        },
        "initComplete": function () {
            ...
            $('#bulk-process-trans').on('click', function () {
                ...
                bulkProcessCreditCard(checkedTransactions, currTable);
            });
            ...
            $('#bulk-delete-trans').on('click', function () {
                ...
                bulkDeleteCreditCard(checkedTransactions, currTable);
            });

            $('#btn-download-template').on('click', function () {
                bulkDownloadTemplateCreditShield();
            });
        }
    });
    return currTable;
...
function processTrans(button) {
    ...
    bootbox.confirm("Are you sure want to process this transaction?", function (result) {
        ...
        if (result) {
            ...
            $.ajax({
                type: 'POST',
                url: 'processsinglcreditcard/' + trans['cardId'],
                dataType: 'json',
                async: true,
...
function bulkProcessCreditCard(cardIds, tableInstance) {
    if (cardIds.length === 0){
        Swal.fire({
            title: 'Error',
...
function bulkDeleteCreditCard(cardIds, tableInstance) {
    if (cardIds.length === 0){
        Swal.fire({
            title: 'Error',
            text: 'Select At least One Transaction To Process',
