...
function getBulkPolicy() {
    var url = 'viewunprocessembedretrench';
    var currTable = $('#loadedPolicyTable').DataTable({
        ...
        "columns": [
            ...
            // },
            {
                "data": "polId",
                "render": function (data, type, full, meta) {
                    return `<button type="button" class="btn btn-primary btn-sm" data-trans=${encodeURI(JSON.stringify(full))} value="Process" onclick="processRetrench(this);">Authorize</button>`;
                }
            }
        ],
        ...
        "initComplete": function () {
            ...
            $('#bulk-authorize-policy').on('click', function () {
                ...
                bulkretrenchAuth(checkedPolicy, currTable);
            });
...
function processRetrench(button) {
    ...
    bootbox.confirm("Are you sure want to authorize this policy?", function (result) {
        ...
        if (result) {
            ...
            $.ajax({
                type: 'POST',
                url: 'approvesingleretrench/' + trans['polId'],
                dataType: 'json',
                async: true,
...
function bulkretrenchAuth(policyId, tableInstance) {
