$(function () {
    $(document).ready(function () {
        importCreditShieldData();
        getUnprocessedCreditShield();
    });
});

// function importCreditShieldData() {
    ...
    $('form#data-upload-form').submit(function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                const uploadmessage = response.message || [];
                ...
                if (uploadstatus === 'Accepted') {
                    ...
                    // If message is an array, join; else just append the message
                    if (Array.isArray(uploadmessage)) {
                        message += uploadmessage.join('\n');
                    } else if (typeof uploadmessage === 'string') {
...
function getUnprocessedCreditShield() {
    var url = "unprocesscreditshield";
    var currTable = $('#uploadedTransTable').DataTable({
        ...
        "initComplete": function () {
                ...
                if(isChecked) {
                    $.ajax({
                        ...
                        success: function (result) {
                            result.forEach(id => checkedRows[id] = true);
                            currTable.rows().every(function () {
                                $(this.node()).find('.rowCheckbox').prop('checked', true);
                            });
                        },
                    ...
                    checkedRows = {};
                    currTable.rows().every(function () {
                        $(this.node()).find('.rowCheckbox').prop('checked', false);
                    });
                }
            ...
            $('#bulk-process-trans').on('click', function () {
                ...
                bulkProcessCreditShield(checkedTransactions, currTable);
            });
            ...
            $('#bulk-delete-trans').on('click', function () {
                ...
                bulkDeleteCreditShield(checkedTransactions, currTable);
            });
            ...
            $('#btn-download-template').on('click', function () {
                ...
                bulkDownloadTemplateCreditCard(checkedTransactions, currTable);
            });
        }
    });
    return currTable;
...
function processTrans(button) {
    ...
    bootbox.confirm("Are you sure want to process this transaction?", function (result) {
        ...
        if (result) {
            ...
            $.ajax({
                type: 'POST',
                url: 'processsinglcreditshield/' + trans['shieldId'],
                dataType: 'json',
                async: true,
                success: function (result) {
                    Swal.fire({
...
function bulkProcessCreditShield(cardIds, tableInstance) {
    if (cardIds.length === 0){
        Swal.fire({
            title: 'Error',
    ...
    bootbox.confirm("Are you sure you want to bulk process selected transactions?", function (result) {
        if(result){
            ...
            $.ajax({
                url: 'processbulkcreditshield',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(cardIds.map(Number)),
                success: function (response) {
                    Swal.fire({
                        title: 'Success',
                        text: 'Batch Job Submitted Successfully. We will alert you once the process is done',
                        icon: 'success'
                    }).then((result) => {
                            UTILITIES.pollJobStatus(response.jobId);
...
function bulkDownloadTemplateCreditShield() {
            $('#btn-download-template').prop('disabled', true);
            $.ajax({
                url: 'absa-bancassurance/protected/upload/templates/embed-creditcard-template',
                type: 'POST',
                contentType: 'application/json',
                success: function (response) {
                    Swal.fire({
...
function bulkDeleteCreditShield(cardIds, tableInstance) {
    if (cardIds.length === 0){
        Swal.fire({
            title: 'Error',
            text: 'Select At least One Transaction To Process',
