$(function () {
    $(document).ready(function () {
        getBulkCreditCardPolicy();
    });
});

function getBulkCreditCardPolicy() {
    var url = 'viewunprocesscreditcard';
    var currTable = $('#loadedPolicyTable').DataTable({
        ...
        "columns": [
            ...
            // },
            {
                "data": "polId",
                "render": function (data, type, full, meta) {
                    return `<button type="button" class="btn btn-primary btn-sm" data-trans=${encodeURI(JSON.stringify(full))} value="Process" onclick="processCreditCard(this);">Authorize</button>`;
                }
            }
        ],
        ...
        "initComplete": function () {
            ...
            $('#bulk-authorize-policy').on('click', function () {
                ...
                bulkCreditCardAuth(checkedPolicy, currTable);
            });
            $('#bulk-delete-policy').on('click', function () {
                var checkedPolicy = Object.keys(checkedRows).filter(function (id) {
                    return checkedRows[id] === true;
                });
                bulkDeleteProcessedCreditCardAuth(checkedPolicy, currTable);
            });
        }
    });
    return currTable;
...
function processCreditCard(button) {
    ...
    bootbox.confirm("Are you sure want to authorize this policy?", function (result) {
        ...
        if (result) {
            ...
            $.ajax({
                type: 'POST',
                url: 'approvesinglercreditcard/' + trans['polId'],
                dataType: 'json',
                async: true,
...
function bulkCreditCardAuth(policyId, tableInstance) {
...
function bulkDeleteProcessedCreditCardAuth(policyId, tableInstance) {
    if (policyId.length === 0){
        Swal.fire({
            title: 'Error',
            text: 'Select At least One Policy To Authorize',
    ...
    bootbox.confirm("Are you sure want to bulk delete selected policies?", function (result) {
        if (result){
            ...
            $.ajax({
                url: 'deleteprocessedbulkcredcard',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(policyId.map(Number)),
                success: function (response) {
