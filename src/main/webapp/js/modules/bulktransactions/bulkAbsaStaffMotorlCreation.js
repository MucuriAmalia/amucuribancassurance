...
function getUnProcessedTransactions() {
    var url = "unProcessedBulkPol";
     currTable = $('#uploadedTransTable').DataTable({
        "processing": true,
        "serverSide": true,
        "autoWidth": true,
        "ajax": {
...
function bulkTemplateDownload() {
        $('#btn-template-download').prop('disabled', true);
        $.ajax({
            url: 'bulktemplatedownload',
            type: 'POST',
            contentType: 'application/json',
            success: function (response) {
                Swal.fire({
            ...
            complete: function () {
                $('#btn-template-download').prop('disabled', false);
            }
        });
}
