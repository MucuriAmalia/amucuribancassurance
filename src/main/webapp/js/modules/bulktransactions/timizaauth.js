...
function getBulkTimizaPolicy() {
    var url = 'viewunprocesstimiza';
    var currTable = $('#loadedPolicyTable').DataTable({
        ...
        "columns": [
            ...
            // },
            {
                "data": "polId",
                "render": function (data, type, full, meta) {
                    return `<button type="button" class="btn btn-primary btn-sm" data-trans=${encodeURI(JSON.stringify(full))} value="Process" onclick="processTimiza(this);">Authorize</button>`;
                }
            }
        ],
        ...
        "initComplete": function () {
            ...
            $('#bulk-authorize-policy').on('click', function () {
                ...
                bulkTimizaAuth(checkedPolicy, currTable);
            });
            $('#bulk-delete-policy').on('click', function () {
                var checkedPolicy = Object.keys(checkedRows).filter(function (id) {
                    return checkedRows[id] === true;
                });
                bulkDeleteProcessedTimiza(checkedPolicy, currTable);
            });
        }
    });
    return currTable;
...
function processTimiza(button) {
    ...
    bootbox.confirm("Are you sure want to authorize this policy?", function (result) {
        console.log(result);
        if (result) {
//            $('#loadedPolicyTable').find('button').prop('disabled', true);
            ...
            $.ajax({
                type: 'POST',
                url: 'approvesingletimiza/' + trans['polId'],
                dataType: 'json',
                async: true,
                ...
                complete: function () {
//                    $('#loadedPolicyTable').find('button').prop('disabled', false);
...
function bulkTimizaAuth(policyId, tableInstance) {
...
function bulkDeleteProcessedTimiza(policyId, tableInstance) {
    if (policyId.length === 0){
        Swal.fire({
            title: 'Error',
            text: 'Select At least One Policy To Authorize',
