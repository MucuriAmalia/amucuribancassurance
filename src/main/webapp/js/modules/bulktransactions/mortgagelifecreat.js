$(function () {
    $(document).ready(function () {
        importMortgageLifeData();
        getUnprocessedMortgageLife();
    });
});

// function importMortgageLifeData() {
    ...
    $('form#data-upload-form').submit(function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                const failedRecords = response.failedRecords || 0;
                const invalidRecords = response.invalidRecords || [];
                const errorFileId = response.errorFileId;
                ...
                // If error file exists, show download button
                if (failedRecords > 0 && errorFileId) {
                    $("#download-error-btn").show().off('click').on('click', function() {
                        window.location.href = ctx + `/protected/mortgage/life/download-error-file/${errorFileId}`;
                    });
                } else {
                    $("#download-error-btn").hide();
                }
                if (successfulPolicies > 0) {
                    $('#uploadedTransTable').DataTable().ajax.reload();
                }
...
function getUnprocessedMortgageLife() {
    var url = "unprocessmortgagelife";
    var currTable = $('#uploadedTransTable').DataTable({
        ...
        "columns": [
            ...
            },
            {
                "data": "mortgageId",
                "render": function (data, type, full, meta) {
                    return `<button type="button" class="btn btn-primary btn-sm" data-trans=${encodeURI(JSON.stringify(full))} value="Process" onclick="processMorgageLife(this);">Process</button>`;
                }
            }
        ],
        "rowCallback": function (row, data) {
            var checkbox = $(row).find('.rowCheckbox');
            checkbox.prop('checked', checkedRows[data.mortgageId] || false);
        },
        "initComplete": function () {
            ...
            $('#bulk-process-trans').on('click', function () {
                ...
                bulkProcessMortgageLife(checkedTransactions, currTable);
            });
            ...
            $('#bulk-delete-trans').on('click', function () {
                ...
                bulkDeleteMortgageLife(checkedTransactions, currTable);
            });
        }
    });
    return currTable;
...
function processMorgageLife(button) {
    ...
    bootbox.confirm("Are you sure want to process this transaction?", function (result) {
        ...
        if (result) {
            ...
            $.ajax({
                type: 'POST',
                url: 'processsinglmortgagelife/' + trans['mortgageId'],
                dataType: 'json',
                async: true,
...
function bulkProcessMortgageLife(mortgageIds, tableInstance) {
    ...
    bootbox.confirm("Are you sure you want to bulk process selected transactions?", function (result) {
        if(result) {
            ...
            $.ajax({
                url: 'processbulkmortgagelife',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(mortgageIds.map(Number)),
                success: function (response) {
                    Swal.fire({
                        title: 'Success',
...
function bulkDeleteMortgageLife(mortgageIds, tableInstance) {
    if (mortgageIds.length === 0){
        Swal.fire({
            title: 'Error',
            text: 'Select At least One Transaction To Process',
