            ...
            success: function (response) {
                ...
                if (failedRecords > 0 && errorFileId) {
                    $("#download-error-btn").show().off('click').on('click', function() {
                        window.location.href = ctx + `/protected/credit/life/download-error-file/${errorFileId}`;
                    });
                } else {
...
function getUnProcessedTransactions() {
    ...
    var currTable = $('#uploadedTransTable').DataTable({
        ...
        "rowCallback": function (row, data) {
            ...
            checkbox.prop('checked', checkedRows[data.creditLifeId] || false);
        },
...
function processTrans(button) {
    ...
    bootbox.confirm("Are you sure want to process this transaction?", function (result) {
        ...
        if (result) {
            ...
            $.ajax({
                type: 'POST',
                url: 'processsinglcreditlife/' + trans['creditLifeId'],
                dataType: 'json',
                async: true,
...
function bulkDeleteTransactions(transactionIds, tableInstance) {
    ...
    bootbox.confirm("Are you sure you want to delete selected transactions?", function (result) {
        if(result) {
            ...
            $.ajax({
                url: 'deletebulkcreditlifeupload',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(transactionIds.map(Number)),
                success: function (response) {
                    Swal.fire({
                        title: 'Success',
                        text: 'Credit life transaction deleted Successfully',
                        icon: 'success'
                    });
                    checkedRows = {};
                    $('#selectAll').prop('checked', false);
