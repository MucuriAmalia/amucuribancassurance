...
function importData() {
    ...
    $('form#data-upload-form').submit(function (e) {
        ...
        // Disable buttons
        $("#upload-btn, #close-btn, #btn-import-data , #bulk-process-trans, #bulk-delete-trans").prop('disabled', true);
        // Show progress bar
            ...
            success: function (response) {
                ...
                if (failedRecords > 0 && errorFileId) {
                    $("#download-error-btn").show().off('click').on('click', function() {
                        window.location.href = ctx + `/protected/endorsement/bulk/download-error-file/${errorFileId}`;
                    });
                } else {
                    $("#download-error-btn").hide();
                }

                // Always try to reload the table if it's initialized
                if ($.fn.DataTable.isDataTable('#uploadedTransTable')) {
                    $('#uploadedTransTable').DataTable().ajax.reload(null, false);
                }

//                if (successfulPolicies > 0 && successfulRisks > 0 && failedRecords === 0) {
//                    $('#uploadedTransTable').DataTable().ajax.reload();
//                }

                // Re-enable buttons
                $("#upload-btn, #close-btn, #btn-import-data, #bulk-process-trans, #bulk-delete-trans").prop('disabled', false);
            },
