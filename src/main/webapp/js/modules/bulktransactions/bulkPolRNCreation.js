    ...
    $('form#data-upload-form').submit(function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // Load error records if any
                if (response.errorCount > 0 && response.batchId) {
                    loadUploadErrors(response.batchId);
                } else {
                    $('#errorTableContainer').hide();
                }
                // Re-enable buttons
...
//                "data": "premium",
            ...
            // },
            {"data": "newInterface"},
            {"data": "accrualPaymentType"},
            {
                "data": "accrualInstDate",
                "render": function (data, type, full, meta) {
                    if (full.accrualInstDate) {
                       return moment(full.accrualInstDate).format('DD/MM/YYYY');
                   } else return "";
                }
            },
            {
                "data": "bulkPolicyRenewalDate",
                "render": function (data, type, full, meta) {
                     if (full.bulkPolicyRenewalDate) {
                        return moment(full.bulkPolicyRenewalDate).format('DD/MM/YYYY');
                    } else return "";
                }
            },
            {
...
function loadUploadErrors(batchId) {
    $.ajax({
        url: 'uploadErrors',
        type: 'GET',
        ...
        error: function (xhr) {
            console.error("Error loading upload errors:", xhr.responseText);
        }
    });
}
    ...
    errors.forEach(function (error) {
        rowsHtml += `
            <tr>
                ...
                <td>${error.policyNumber}</td>
                <td>${error.proposalNumber}</td>
                <td>${error.errorMessage}</td>
            </tr>
        `;
    });

    $('#errorTableBody').html(rowsHtml);
    $('#errorTableContainer').show();
}
