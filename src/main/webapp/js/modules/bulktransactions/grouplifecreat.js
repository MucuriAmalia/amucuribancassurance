...
//                     message += `Uploaded Risk Details: ${successfulRisks}\n\n`;
    ...
    var currTable = $('#uploadedTransTable').DataTable({
        ...
        "columns": [
            ...
            },
            {
                ...
                "render": function (data, type, full, meta) {
                    if (full.uploadDate) {
                        return moment(full.uploadDate).format('DD/MM/YYYY');
                    } else return "No Date";
                }
            },
            {
        ...
        "rowCallback": function (row, data) {
            ...
            checkbox.prop('checked', checkedRows[data.groupLifeId] || false);
        },
...
function processTrans(button) {
    ...
    bootbox.confirm("Are you sure want to process this transaction?", function (result) {
        ...
        if (result) {
            ...
            $.ajax({
                type: 'POST',
                url: 'processsinglgrouplife/' + trans['groupLifeId'],
                dataType: 'json',
                async: true,
