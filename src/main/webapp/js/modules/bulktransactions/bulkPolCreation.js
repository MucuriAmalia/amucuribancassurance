    ...
    $('form#data-upload-form').submit(function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                const successfulPolicies = response.successfulPolicies || 0;
                const successfulRisks = response.successfulRisks || 0;
                const failedRecords = response.totalFailedPolicyCount+response.totalFailedRiskCount || 0;
                const invalidRecords = response.invalidRecords || [];
                ...
                if (failedRecords > 0 && errorFileId) {
                    $("#download-error-btn").show().off('click').on('click', function() {
                        window.location.href = ctx + `/protected/bulk/policycreation/download-error-file/${errorFileId}`;
                    });
                } else {
