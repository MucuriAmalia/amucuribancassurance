...
public interface BulkStaffMotorsCreationRepo extends PagingAndSortingRepository<BulkStaffMotorsCreation, Long>, QueryDslPredicateExecutor<BulkStaffMotorsCreation> {

    @Query(value = "SELECT sbcbp.cover_type, sbcbp.bulk_policy_insured_name, sbcbp.motor_registration_no, sbcbp.motor_make_model, \n" +
            "sbcbp.motor_yom, sbcbp.motor_policy_wef, sbcbp.bulk_policy_date_uploaded, sbcbp.bulk_policy_id, COUNT(*) OVER() as total_rows, \n" +
            "sbcbp.motor_sum_insured  \n"+
            "FROM sys_brk_bulk_staff_motors sbcbp \n" +
            "WHERE sbcbp.bulk_policy_processed = 'N' \n" +
            "AND sbcbp.bulk_policy_uploaded_by = :currentUserId \n" +
            "AND (LOWER(sbcbp.bulk_policy_insured_name) like LOWER(:search) \n" +
            "OR LOWER(sbcbp.motor_registration_no) like LOWER(:search) \n" +
            "OR LOWER(sbcbp.motor_make_model) like LOWER(:search) \n" +
            "OR LOWER(sbcbp.motor_yom) like LOWER(:search) \n" +
            "OR LOWER(sbcbp.cover_type) like LOWER(:search) \n" +
            "OR CAST(sbcbp.bulk_policy_date_uploaded AS TEXT) like :search \n" +
            "OR CAST(sbcbp.motor_policy_wef AS TEXT) like :search) \n" +
    ...
    @Query(value = "SELECT sbp.pol_id,sbc2.cov_desc,sbp.pol_no,\n" +
            ...
            "INNER JOIN sys_brk_bulk_staff_motors sbcbp ON sbcbp.bulk_policy_pol_id = sbp.pol_id \n" +
  ...
  //          "WHERE  sbcbp.bulk_policy_authorized = 'N' \n" +
    ...
    @Query(value = "select  * from sys_brk_bulk_staff_motors where bulk_policy_pol_id = :id", nativeQuery = true)
    BulkStaffMotorsCreation findBulkStockByPolicyId(@Param("id") Long id);

//    @Query(value = "select  * from sys_brk_bulk_staff_motors where bulk_policy_pol_id = :bulkId", nativeQuery = true)
//    BulkStaffMotorsCreation findWithPolicyByPolicyId(@Param("bulkId") Long bulkId);
}
